...
public class ProlongedAuction implements Auction
{
    ...
    private RoundState          round_state = RoundState.UNCHANGED;
    ...
    public ProlongedAuction(String name, String description, double price, int round, int minimal_trust)
    {
        this.name = name;
    ...
    public ProlongedAuction(Auctioneer auctioneer, String name, String description, double price, int round, int minimal_trust)
    {
    ...
    public RoundState getRound_state()
    {
        return round_state;
    }

    public void setRound_state(RoundState round_state)
    {
    ...
    public void setRound(int round)
    {
        ...
        {
            if (round == 0 && round_state == RoundState.CHANGED)
            {
                round_state = RoundState.UNCHANGED;
                ReadyToBid();
            }
            else if (round == 0 && round_state == RoundState.UNCHANGED)
            {
            ...
            if (command.equals("next")) {
                ...
                for (Observer b : this.getBidders())
                {
                    ...
                    else
                    {
                        ...
                        b.Bid(bid);
                        if (x != price)
                            round_state = RoundState.CHANGED;
                    }
